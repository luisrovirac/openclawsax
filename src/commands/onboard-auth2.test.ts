...
describe("setGroqApiKey", () => {
  ...
  it("writes to OPENCLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("openclaw-groq-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);

    await setGroqApiKey("gsk-test-key");

    const parsed = await readAuthProfilesForAgent<{
      profiles?: Record<string, { type?: string; provider?: string; key?: string }>;
    }>(env.agentDir);
    expect(parsed.profiles?.["groq:default"]).toMatchObject({
      type: "api_key",
      provider: "groq",
      key: "gsk-test-key",
    });
...
describe("applyGroqConfig", () => {
  it("adds Groq provider with correct settings", () => {
    const cfg = applyGroqConfig({});
    expect(cfg.models?.providers?.groq).toMatchObject({
  ...
  it("includes all Groq models", () => {
    const cfg = applyGroqConfig({});
    const models = cfg.models?.providers?.groq?.models ?? [];
    const modelIds = models.map(m => m.id);
    
    expect(modelIds).toContain("llama3-70b-8192");
    expect(modelIds).toContain("llama3-8b-8192");
    expect(modelIds).toContain("mixtral-8x7b-32768");
  });
});

describe("applyGroqProviderConfig", () => {
  it("merges Groq models and keeps existing provider overrides", () => {
    const cfg = applyGroqProviderConfig(
      createLegacyProviderConfig({
        providerId: "groq",
        api: "anthropic-messages",
    ...
    expect(cfg.models?.providers?.groq?.baseUrl).toBe("https://api.groq.com/openai/v1");
    expect(cfg.models?.providers?.groq?.api).toBe("openai-completions");
    expect(cfg.models?.providers?.groq?.apiKey).toBe("custom-key");
    expect(cfg.models?.providers?.groq?.models.map((m) => m.id)).toEqual([
      "custom-model",
      "llama3-70b-8192",
      "llama3-8b-8192",
      "mixtral-8x7b-32768",
  ...
  it("adds expected alias for Groq provider", () => {
    const cfg = applyGroqProviderConfig({});
    expect(cfg.agents?.defaults?.models?.[GROQ_DEFAULT_MODEL_REF]?.alias).toBe("Llama 3");
  });
});

describe("provider config helpers", () => {
  it("does not overwrite existing primary model", () => {
    const providerConfigAppliers = [applyMinimaxApiProviderConfig, applyZaiProviderConfig, applyGroqProviderConfig];
...
describe("primary model defaults", () => {
  it("sets correct primary model", () => {
    const configCases = [
      ...
      {
        getConfig: () => applyGroqConfig({}),
        primaryModel: GROQ_DEFAULT_MODEL_REF,
      },
    ] as const;
...
describe("fallback preservation helpers", () => {
  it("preserves existing model fallbacks", () => {
    const fallbackCases = [applyMinimaxApiConfig, applyXaiConfig, applyMistralConfig, applyGroqConfig] as const;
...
describe("provider alias defaults", () => {
  it("adds expected alias for provider defaults", () => {
    const aliasCases = [
      ...
      {
        applyConfig: () => applyGroqProviderConfig({}),
        modelRef: GROQ_DEFAULT_MODEL_REF,
        alias: "Llama 3",
      },
    ] as const;
